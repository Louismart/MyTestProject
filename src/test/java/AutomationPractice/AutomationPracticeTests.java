...
import automationTestFramework.pages.TextInput;
...
//        WebElement contactUsLocator = wait.until(ExpectedConditions.visibilityOfElementLocated(CustomerPage.contactUsLocator));
    ...
    @Test
    public void userVerifyButtonsPopularAndBestSellersProductsection() {
...
//        bestSellers.click();
        testPage.bestSellersButton().get(0).click();

         Assert.assertFalse(liActive.get(0).getAttribute("class").equals("active"), "Verifying if Popular is not active");
         Assert.assertTrue(liActive.get(1).getAttribute("class").equals("active"), "Verifying if bestSellers is active");
    ...
    public void userVerifySearchField() {
         ...
         */
        String result = "";
         testPage.search().sendKeys("faded short");
